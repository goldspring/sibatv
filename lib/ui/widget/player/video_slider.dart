...
class _VideoSliderState extends State<VideoSlider>
  ...
  void _actionHandler(_AdjustSliderIntent intent) {
    ...
    switch (intent.type) {
      case _SliderAdjustmentType.right:
        widget.controller?.gotFocus(focus: true);
        _hideTimer?.cancel();
        _hideTimer = Timer(const Duration(seconds: 1), () {
          renderSlider.indecreaseActionEnd();
        });
  ...
  void _handleFocusHighlightChanged(bool focused) {
    if (focused != _focused) {
      widget.controller?.gotFocus(focus: focused);
      setState(() {
        _focused = focused;
      });
    }
...
class VideoSliderController extends ChangeNotifier {
...
class _RenderSlider extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  _RenderSlider({
    ...
    required this.onChangeStart,
    required this.onChangeEnd,
    required _VideoSliderState state,
  ...
  final _VideoSliderState _state;
  ...
  void increaseAction() {
    if (isInteractive) {
      _posVal = (value + _semanticActionUnit).clamp(0.0, 1.0);
      onChanged!(_posVal);
    }
  }

  void indecreaseActionEnd() {
    if (isInteractive) {
      onChangeEnd!(_posVal);
    }
  }
