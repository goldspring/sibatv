...
class LivesNotifier extends StateNotifier<List<LiveData>> {
  final bool inLive;
  LivesNotifier({required this.inLive}) : super([]) {
    if (kDebugMode) {
      print("LivesNotifier Constructor");
    }
  }
  ...
  void _liveData(String next, LiveData datas) {
    if (next == "0") {
      state = [datas];
    } else {
      state = [...state, datas];
  ...
  static Future<LiveRoomInfo> requestLiveRoomInfo(String id) {
    final url =
        Uri.parse('https://pocketapi.48.cn/live/api/v1/live/getLiveOne');
    return http
        .post(url,
            headers: _createHeaders(), body: json.encode({'liveId': '${id}'}))
        .then((response) {
  ...
  */
  void requestLiveList(
    ...
    final res = await http
        .post(url, headers: _createHeaders(), body: json.encode(body))
        .then((response) {
      print("Response status: ${response.statusCode}");
      print("Response body: ${response.body}");
      if (response.statusCode != 200) {
      ...
      _liveData(next, LiveData.fromJson(json.decode(response.body)));
  ...
  static Map<String, String> _createHeaders() {
    return {
      ...
      'appInfo': jsonEncode({
        'vendor': 'apple',
        'deviceId':
            '${_rStr(8)}-${_rStr(4)}-${_rStr(4)}-${_rStr(4)}-${_rStr(12)}',
        'appVersion': '6.2.2',
        'appBuild': '21080401',
        'osVersion': '11.4.1',
        'osType': 'ios',
