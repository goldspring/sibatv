...
public class RecorderThread implements Runnable {
    ...
    public RecorderThread(MediaProjection mediaProjection, FileDescriptor outputFD,
                          int videoWidth, int videoHeight, int videoBitrate) {
        this.mediaProjection = mediaProjection;
        handler = new Handler();
        this.outputFileDescriptor = outputFD;
        ...
        System.out.println("HHRecorder Video width:" + videoWidth);
        System.out.println("HHRecorder Video Height:" + videoHeight);
        System.out.println("HHRecorder Video Bitrate:" + videoBitrate);
    ...
    private void setupVideoCodec() throws IOException {
        ...
        encoderFormat.setLong(MediaFormat.KEY_REPEAT_PREVIOUS_FRAME_AFTER,  1000000L / 30);
        encoderFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, 5);



        String codecName = new MediaCodecList(MediaCodecList.REGULAR_CODECS).findEncoderForFormat(encoderFormat);
        System.out.println("Recorder Video Name:" + codecName);
        if (codecName != null) {
            videoEncoder = MediaCodec.createByCodecName(codecName);
        } else {
            videoEncoder = MediaCodec.createEncoderByType(videoMime);
        }

        System.out.println("ENCODER Video Name:" + videoEncoder.getName());
    ...
    private void setupAudioCodec() throws IOException {
        ...
        MediaFormat encoderFormat = MediaFormat.createAudioFormat(MediaFormat.MIMETYPE_AUDIO_AAC, sampleRate, 2);
    ...
    private void setupAudioRecord() {
        ...
        AudioFormat newAudioFormat = audioFormatBuilder.setEncoding(AudioFormat.ENCODING_PCM_16BIT).setSampleRate(sampleRate).setChannelMask(AudioFormat.CHANNEL_IN_STEREO).build();
    ...
    public void run() {
        try {
            ...
            if (this.outputFilePath != null) {
                ...
                muxer = new MediaMuxer(this.outputFileDescriptor,
                        MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
            ...
            while (!stopped && !asyncError) {
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    Log.w(TAG, "encoder.dequeueOutputBuffer: setup");
                    startMuxerIfSetUp();
                } else if (encoderStatus < 0) {
                    Log.w(TAG, "unexpected result from encoder.dequeueOutputBuffer: " + encoderStatus);
                } else {
                    Log.w(TAG, "encoder.dequeueOutputBuffer: buffer");
