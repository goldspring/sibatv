...
public class HhScreenRecorderPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware, PluginRegistry.ActivityResultListener{
  ...
  void supportGL(){
    ConfigurationInfo configurationInfo = ((ActivityManager) Objects.requireNonNull(m_activity.getSystemService(Context.ACTIVITY_SERVICE))).getDeviceConfigurationInfo();
    StringBuilder sb = new StringBuilder();
    sb.append("GL version:").append(configurationInfo.getGlEsVersion()).append("\n");
    System.out.println("GL version:" +configurationInfo.getGlEsVersion());
  }
  void startRecording()
  {
  ...
  void stopRecording()
  {
    if(m_recordingState == RecordingState.Recording || m_recordingState == RecordingState.Paused)
    {
      ...
      //Intent service = new Intent(m_context, RecorderService.class);
      ...
      // dataMap.put("msg", msg);
      // JSONObject jsonObj = new JSONObject(dataMap);
      results.put("filename", m_finalFullPath);
      m_flutterResult.success(results);
    }
    else
  ...
  public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == SCREEN_RECORD_REQUEST_CODE) {
      if (resultCode == Activity.RESULT_OK) {
        if (data != null) {
          System.out.println("HHRecorder: StartService(onActivitiyResult)");
  ...
  private void startService(int code, Intent data)
  {
    try
    {
      ...
      //service.putExtra(RecorderService.EXTRA_RESULT_CODE, 1001);
      service.putExtra(ScreenCaptureService.EXTRA_RESULT_CODE, 1001);

      /*
      service = new Intent(m_context, RecorderService.class);
      service.setAction(RecorderService.ACTION_START);
      service.putExtra(RecorderService.EXTRA_RESULT_CODE, 1001);
  ...
  public void onFailedToStartCapture(String reason)
  {
    ...
    try {
      ...
      //Intent mService = new Intent(m_context, RecorderService.class);
  ...
  */
  ...
  private void checkAddContentValues()
  {
    ...
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q)
      m_contentValues.put(MediaStore.Video.Media.RELATIVE_PATH, "Movies/");
    else
  ...
  private String getDateAndTime(){
    ...
    @SuppressLint("SimpleDateFormat") DateFormat dfTime = new SimpleDateFormat("HHmm-SSS");
